<?php
    ...
    function aplicarFormatoTelefono(?string $valor, ?string $formato, ?string $prefijo, int $digitos = 0): string {
        ...
        if ($formato === null || $formato === '') return trim(($prefijo ?? '') . ' ' . $valor);
        $resultado = trim((string) $prefijo);
        if ($resultado !== '') $resultado .= ' ';
        $indice = 0;
        foreach (str_split($formato) as $caracter) {
            if ($caracter === '#') {
                if (!isset($valor[$indice])) break;
    ...
    function aplicarFormatoDocumento(?string $valor, ?string $formato): string {
        if ($valor === null || $valor === '') return '';
        $valor = preg_replace('/\D/', '', $valor) ?? '';
        if ($formato === null || $formato === '') return $valor;
        $resultado = '';
        $indice = 0;
        foreach (str_split($formato) as $caracter) {
            if ($caracter === '#') {
    ...
    function obtenerBanderaPais(?string $iso2Code): string {
        $iso2Code = strtoupper(trim((string) $iso2Code));
        if (strlen($iso2Code) !== 2 || !ctype_alpha($iso2Code)) return '';
        return mb_chr(127397 + ord($iso2Code[0]), 'UTF-8') . mb_chr(127397 + ord($iso2Code[1]), 'UTF-8');
    }
    ...
    $clientes = array_values(array_filter($clientes, static function (array $cliente) use ($filters, $addressesByEntity): bool {
        $matches = static function (?string $value, string $filter): bool { return $filter === '' || mb_stripos((string) $value, $filter) !== false; };
        $entityId = (int) $cliente['id'];
        $addressValues = [(string) ($cliente['address'] ?? '')];

        foreach ($addressesByEntity[$entityId] ?? [] as $address) {
            $addressValues[] = implode(' ', array_filter([
        ...
        return $matches($cliente['name'] ?? '', $filters['name'])
            ...
            && $matches(implode(' ', $addressValues), $filters['address']);
    }));
    ...
    if ($clientes !== []) {
        ...
        while ($row = $contactStmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (!empty($row['isPrimary']) && !isset($primaryContacts[$entityId][$typeId])) $primaryContacts[$entityId][$typeId] = $row['contact'];
        }
    }
    ...
    if (hasPermission('CLIENT_EDIT')) {
        $contactsTooltip = "Administrar Contactos";
    }
    elseif (hasPermission('CLIENT_VIEW')) {
        $contactsTooltip = "Ver Contactos";
    }
...
<div class="toast-container position-fixed top-0 end-0 p-3" style="z-index: 1090; margin-top: 65px;">
    <div id="successToast" class="toast text-bg-success border-0" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="d-flex">
            <div class="toast-body"><i class="bi bi-check-circle-fill me-2"></i><?= htmlspecialchars($updateMessage, ENT_QUOTES, 'UTF-8') ?></div><button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast" aria-label="Cerrar"></button>
        </div>
    </div>
</div>
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="page-title mb-0">Clientes</h1><a href="<?= BASE_URL ?>/private/entidades/clientes/new.php" class="btn btn-primary"><i class="bi bi-plus-circle me-1"></i>Nuevo cliente</a>
</div>
        ...
        <div class="table-responsive">
            <table id="clientCollapses" class="table table-striped table-hover align-middle">
                ...
                <tbody>
                    ...
                    <?php if ($clientes === []): ?>
                    <tr>
                        <td colspan="<?= count($contactTypes) + 6 ?>" class="text-center text-body-secondary py-4">No se encontraron clientes.</td>
                    </tr>
                    <?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?= htmlspecialchars(aplicarFormatoDocumento($cliente['documentNumber'] ?? null, $cliente['documentFormat'] ?? null), ENT_QUOTES, 'UTF-8') ?>
                        </td>

                        <?php foreach ($contactTypes as $ct): ?>
                        <td>
                            <?= htmlspecialchars((string) ($primaryContacts[$entityId][(int) $ct['id']] ?? '---'), ENT_QUOTES, 'UTF-8') ?>
                        </td>
                        <?php endforeach; ?>
                        ...
                        <td>
                            ...
                            <?php if (!empty($primaryAddress['cityName'])): ?>
                                <div class="small text-body-secondary"><?= htmlspecialchars((string) $primaryAddress['cityName'], ENT_QUOTES, 'UTF-8') ?></div>
                            <?php endif; ?>
                        ...
                        <td class="text-end">
                            ...
                            <div class="client-actions">
                                <div class="client-action-row">
                                    <span>
                                    <button type="button" class="btn btn-xs btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#<?= $contactsCollapseId ?>" data-tooltip="<?= $contactsTooltip ?>" aria-expanded="false" aria-controls="<?= $contactsCollapseId ?>">
                                        <i class="bi bi-telephone"></i>
                                    </button>
                                    </span>
                                    <span>
                                    <button type="button" class="btn btn-xs btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#<?= $addressesCollapseId ?>" data-tooltip="Administrar direcciones" aria-expanded="false" aria-controls="<?= $addressesCollapseId ?>">
                                        <i class="bi bi-geo-alt"></i>
                                    </button>
                                    </span>
                                    <span>
                                    <button type="button" class="btn btn-xs btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#<?= $commentsCollapseId ?>" data-tooltip="Ver comentarios" aria-expanded="false" aria-controls="<?= $commentsCollapseId ?>" <?= !$hasComments ? 'disabled' : '' ?>>
                                    ...
                                    <span>
                                    <a class="btn btn-xs btn-outline-secondary" href="<?= BASE_URL ?>/private/importaciones/index.php?entityId=<?= $entityId ?>" data-bs-toggle="tooltip" title="Ver importaciones"><i class="bi bi-box-seam"></i></a>
                                    </span>
                                    <?php endif; ?>
                                ...
                                <div class="client-action-row">
                                    ...
                                    <?php if (hasPermission('CLIENT_EDIT')): ?>
                                        <span>
                                        <a class="btn btn-xs btn-outline-warning" href="edit.php?id=<?= $entityId ?>" data-bs-toggle="tooltip" title="Editar cliente"><i class="bi bi-pencil-square"></i></a>
                                        </span>
                                    <?php endif; ?>

                                    <?php if (hasPermission('USER_VIEW')): ?>
                                    <span>
                                        <button type="button" class="btn btn-xs btn-outline-primary" data-bs-toggle="collapse" data-bs-target="#<?= $usersCollapseId ?>" data-tooltip="Administrar usuarios" aria-expanded="false" aria-controls="<?= $usersCollapseId ?>"><i class="bi bi-person-gear"></i></button>
                                    </span>
                                    <?php endif; ?>

                                    <?php if (hasPermission('ENTITY_DELETE')): ?>
                                    <span>
                                        <form method="POST" class="d-inline"><?= csrfField() ?><button type="submit" name="borrar" value="<?= $entityId ?>" class="btn btn-xs btn-outline-danger" data-bs-toggle="tooltip" title="Borrar cliente" onclick="return confirm('¿Borrar este cliente?');"><i class="bi bi-person-dash"></i></button></form>
                                    </span>
                                    <?php endif; ?>
                    ...
                    <tr class="contacts-detail-row">
                        <td colspan="<?= count($contactTypes) + 6 ?>" class="p-0 border-0">
                            <div id="<?= $contactsCollapseId ?>" class="collapse" data-bs-parent="#clientCollapses">
                                <div class="p-3 border-bottom bg-body-tertiary">
                                    <div class="d-flex justify-content-between align-items-center mb-3"><strong>Contactos</strong>
                                        <?php if (hasPermission('CLIENT_EDIT')): ?>
                                        <a href="<?= BASE_URL ?>/private/entidades/contactos/new.php?entityId=<?= $entityId ?>&modal=1" class="btn btn-xs btn-primary entity-modal-trigger" data-bs-toggle="modal" data-bs-target="#entityFormModal" data-modal-title="Nuevo contacto">
                                    ...
                                    <?php if ($contacts === []): ?>
                                    <div class="alert alert-secondary mb-0"><i class="bi bi-info-circle me-1"></i>Este cliente no tiene contactos registrados.</div>
                                    <?php else: ?>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-sm table-hover align-middle mb-0">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <?= htmlspecialchars((string) ($contact['type_name'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                                    </td>
                                                    <td>
                                                        <?= htmlspecialchars((string) ($contact['contact'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                                    </td>
                                                    <td>
                                                        <?= htmlspecialchars((string) ($contact['comment'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                                    </td>
                                                    <td>
                                                        <?php if (!empty($contact['isPrimary'])): ?>
                                                            <span class="badge text-bg-success">Principal</span>
                                                        <?php endif; ?>

                                                        <?php if (empty($contact['isActive'])): ?>
                                                            <span class="badge text-bg-secondary">Inactivo</span>
                                                        <?php endif; ?>

                                                        <?php if (!empty($contact['isDeleted'])): ?>
                                                            <span class="badge text-bg-danger">Borrado</span>
                                                        <?php endif; ?>
                                                    </td>
                                                    <td class="text-end text-nowrap">
                                                        <?php if (empty($contact['isDeleted']) && hasPermission('CLIENT_EDIT')): ?>
                                                        <a href="<?= BASE_URL ?>/private/entidades/contactos/edit.php?id=<?= $contactId ?>&modal=1" class="btn btn-xs btn-outline-warning entity-modal-trigger" data-bs-toggle="modal" data-bs-target="#entityFormModal" data-tooltip="Editar contacto" title="Editar contacto">
                                                            <i class="bi bi-pencil-square"></i>
                                                        </a>
                                                        <form method="POST" class="d-inline"><?= csrfField() ?>
                                                            <button type="submit" name="deleteContact" value="<?= $contactId ?>" class="btn btn-xs btn-outline-danger" data-bs-toggle="tooltip" title="Eliminar contacto" onclick="return confirm('¿Eliminar este contacto?');">
                    ...
                    <tr class="addresses-detail-row">
                        <td colspan="<?= count($contactTypes) + 6 ?>" class="p-0 border-0">
                            <div id="<?= $addressesCollapseId ?>" class="collapse" data-bs-parent="#clientCollapses">
                                <div class="p-3 border-bottom bg-body-tertiary">
                                    <div class="d-flex justify-content-between align-items-center mb-3"><strong>Direcciones</strong>
                                        <?php if (hasPermission('CLIENT_EDIT')): ?>
                                            <a href="<?= BASE_URL ?>/private/entidades/direcciones/new.php?entityId=<?= $entityId ?>&modal=1" class="btn btn-xs btn-primary entity-modal-trigger" data-bs-toggle="modal" data-bs-target="#entityFormModal" data-modal-title="Nueva dirección">
                                                <i class="bi bi-plus-circle me-1"></i>
                                                Nueva dirección
                                            </a>
                                        <?php endif; ?>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-sm table-hover align-middle mb-0">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <?= htmlspecialchars((string) ($address['addressType'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                                    </td>
                                                    <td>
                                                        <div>
                                                            <?= htmlspecialchars((string) ($address['addressLine1'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                                        </div>
                                                        <?php if (!empty($address['addressLine2'])): ?>
                                                        <div class="small text-body-secondary"><?= htmlspecialchars((string) $address['addressLine2'], ENT_QUOTES, 'UTF-8') ?></div>
                                                        <?php endif; ?>
                                                    </td>
                                                    <td>
                                                        <?= htmlspecialchars(trim(implode(', ', array_filter([$address['postalCode'] ?? '', $address['cityName'] ?? '', $address['stateName'] ?? '', $address['countryName'] ?? '']))), ENT_QUOTES, 'UTF-8') ?>
                                                    </td>
                                                    <td>
                                                        <?php if (!empty($address['isPrimary'])): ?>
                                                        <span class="badge text-bg-success">Principal</span>
                                                        <?php endif; ?>

                                                        <?php if (empty($address['isActive'])): ?>
                                                        <span class="badge text-bg-secondary">Inactiva</span>
                                                        <?php endif; ?>

                                                        <?php if (!empty($address['isDeleted'])): ?>
                                                        <span class="badge text-bg-danger">Borrada</span>
                                                        <?php endif; ?>
                                                    </td>
                                                    <td class="text-end text-nowrap">
                                                        <?php if (empty($address['isDeleted']) && hasPermission('CLIENT_EDIT')): ?>
                                                        <a href="<?= BASE_URL ?>/private/entidades/direcciones/edit.php?id=<?= $addressId ?>&modal=1" class="btn btn-xs btn-outline-warning entity-modal-trigger" data-bs-toggle="modal" data-bs-target="#entityFormModal" data-modal-title="Editar dirección" title="Editar dirección">
                                                            <i class="bi bi-pencil-square"></i>
                                                        </a>
                                                        <form method="POST" class="d-inline"><?= csrfField() ?>
                                                            <button type="submit" name="deleteAddress" value="<?= $addressId ?>" class="btn btn-xs btn-outline-danger" data-bs-toggle="tooltip" title="Eliminar dirección" onclick="return confirm('¿Eliminar esta dirección?');">
                                                                <i class="bi bi-trash"></i>
                                                            </button>
                                                        </form>
                                                        <?php else: ?>
                    ...
                    <tr class="comments-detail-row">
                        <td colspan="<?= count($contactTypes) + 6 ?>" class="p-0 border-0">
                            <div id="<?= $commentsCollapseId ?>" class="collapse" data-bs-parent="#clientCollapses">
                                <div class="p-3 border-bottom bg-body-tertiary">
                                    <div class="d-flex justify-content-between align-items-center mb-2"><strong>Comentarios</strong>
                                        <?php if (hasPermission('CLIENT_EDIT')): ?>
                                        <a href="edit.php?id=<?= $entityId ?>#comments" class="btn btn-xs btn-outline-warning">
                    ...
                    <tr class="users-detail-row">
                        <td colspan="<?= count($contactTypes) + 6 ?>" class="p-0 border-0">
                            <div id="<?= $usersCollapseId ?>" class="collapse" data-bs-parent="#clientCollapses">
                                <div class="p-3 border-bottom bg-body-tertiary">
                                    <div class="d-flex justify-content-between align-items-center mb-3"><strong>Usuarios relacionados</strong>
                                        <?php if (hasPermission('USER_CREATE')): ?>
                                        <a href="<?= BASE_URL ?>/private/entidades/usuarios/new.php?entityId=<?= $entityId ?>&modal=1" class="btn btn-xs btn-primary user-modal-trigger" data-bs-toggle="modal" data-bs-target="#userFormModal"><i class="bi bi-person-plus me-1"></i>Nuevo usuario</a>
                                        <?php endif; ?>
                                    </div>

                                    <?php if ($relatedUsers === []): ?>
                                    <div class="alert alert-secondary mb-0"><i class="bi bi-info-circle me-1"></i>Este cliente no tiene usuarios relacionados.</div>
                                    <?php else: ?>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="userFormModalLabel">Usuario</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body p-0"><iframe id="userFormFrame" title="Formulario de usuario" class="w-100 border-0" style="height: 560px;"></iframe></div>
        </div>
    </div>
</div>
<div class="modal fade" id="entityFormModal" tabindex="-1" aria-labelledby="entityFormModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="entityFormModalLabel">Formulario</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body p-0"><iframe id="entityFormFrame" title="Formulario" class="w-100 border-0" style="height: 560px;"></iframe></div>
        </div>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (userModal && userFrame) {
            userModal.addEventListener('show.bs.modal', function(event) {
                ...
                userFrame.src = trigger ? trigger.getAttribute('href') : '';
            });
        ...
        if (entityModal && entityFrame) {
            entityModal.addEventListener('show.bs.modal', function(event) {
                const trigger = event.relatedTarget;
                entityFrame.src = trigger ? trigger.getAttribute('href') : '';
                entityModalTitle.textContent = trigger?.dataset.modalTitle || 'Formulario';
            });
            entityModal.addEventListener('hidden.bs.modal', function() {
                entityFrame.src = '';
            });
        }
        ...
        window.addEventListener('message', function(event) {
            ...
            if (event.data.type === 'closeUserModal' && userModal) {
                bootstrap.Modal.getOrCreateInstance(userModal).hide();
                window.location.reload();
            }
            if (event.data.type === 'closeEntityModal' && entityModal) {
                bootstrap.Modal.getOrCreateInstance(entityModal).hide();
                window.location.reload();
            }
        });
        ...
        document.querySelectorAll('[data-tooltip]').forEach(function(element) {
            new bootstrap.Tooltip(element, {
                ...
                popperConfig: function(config) {
                    config.modifiers = config.modifiers.map(function(modifier) {
        ...
        if (toastElement) new bootstrap.Toast(toastElement, {
            autohide: true,
            delay: 4000
        }).show();
...
<style>
    ...
    .addresses-detail-row>td,
    .comments-detail-row>td {
