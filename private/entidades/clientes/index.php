<?php
    ...
    */
    ...
    $clientes = getEntitiesByTypeId($pdo, 2, $isAdmin);
            ...
            <div class="table-responsive">
                    ...
                    <thead>
                        ...
                        <tr>
                            ...
                            <th>
                                <input form="clientFilters" type="text" name="email" class="form-control form-control-sm" placeholder="Filtrar..." value="<?= htmlspecialchars($filters['email'], ENT_QUOTES, 'UTF-8') ?>">
                            </th>
                            <th>
                                <input form="clientFilters" type="text" name="phone" class="form-control form-control-sm" placeholder="Filtrar..." value="<?= htmlspecialchars($filters['phone'], ENT_QUOTES, 'UTF-8') ?>">
                            </th>
                            <th>
                    ...
                    <tbody>
                        <?php if ($clientes === []): ?>
                            <tr>
                                <td colspan="<?= $isAdmin ? 10 : 9 ?>" class="text-center text-body-secondary py-4">
                        ...
                        <?php foreach ($clientes as $cliente): ?>
                                        ...
                                        <div class="client-actions">
                                            <div class="client-action-row">
                                                ...
                                                <?php if (hasPermission('USER_VIEW')): ?>
                                                    <button type="button" class="btn btn-xs btn-outline-primary" data-bs-toggle="collapse" data-bs-target="#<?= $collapseId ?>" aria-expanded="false" aria-controls="<?= $collapseId ?>" title="Ver usuarios relacionados">
                                                        <i class="bi bi-person-gear"></i>
                                                    </button>
                                                <?php endif; ?>

                                                <button type="button" class="btn btn-xs btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#<?= $addressCollapseId ?>" aria-expanded="false" aria-controls="<?= $addressCollapseId ?>" title="Ver otras direcciones" <?= $otherAddresses === [] ? 'disabled' : '' ?>>
                                                    <i class="bi bi-geo-alt"></i>
                                                </button>
                                            </div>

                                            <div class="client-action-row">
                                                <button type="button" class="btn btn-xs btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#<?= $commentsCollapseId ?>" aria-expanded="false" aria-controls="<?= $commentsCollapseId ?>" title="Ver comentarios" <?= !$hasComments ? 'disabled' : '' ?>>
                            ...
                            <?php if ($hasComments): ?>
                                <tr class="comments-detail-row">
                                    <td colspan="<?= $isAdmin ? 10 : 9 ?>" class="p-0 border-0">
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <?php if (empty($user['isDeleted'])): ?>
                                                                            <span class="badge <?= !empty($user['isConfirmed']) ? 'text-bg-success' : 'text-bg-secondary' ?>">
                                                                                <?= !empty($user['isConfirmed']) ? 'Confirmado' : 'No confirmado' ?>
                                                                            </span>
                                                                            <span class="badge <?= !empty($user['isBlocked']) ? 'text-bg-danger' : 'text-bg-success' ?>">
                                                                                <?= !empty($user['isBlocked']) ? 'Bloqueado' : 'No bloqueado' ?>
                                                                            </span>
                                                                        <?php endif; ?>
                                                                        <?php if ($isAdmin): ?>
                                                                            <?php if (!empty($user['isDeleted'])): ?>
                                                                    ...
                                                                    <td class="text-end text-nowrap">
                                                                        ...
                                                                        <?php else: ?>
                                                                            <?php if (hasPermission('USER_EDIT')): ?>
                                                                                <a href="<?= BASE_URL ?>/private/entidades/usuarios/edit.php?id=<?= $userId ?>&modal=1" class="btn btn-xs btn-outline-warning user-modal-trigger" data-bs-toggle="modal" data-bs-target="#userFormModal" title="Editar usuario">
                                                                                    <i class="bi bi-pencil-square"></i>
                                                                                </a>
                                                                            <?php endif; ?>
                                                                            ...
                                                                            <?php if (hasPermission('USER_UNLOCK')): ?>
                                                                                <?php if (!empty($user['isBlocked'])): ?>
                                                                                    <form method="POST" class="d-inline">
                                                                                        <?= csrfField() ?>
                                                                                        <button type="submit" name="unblockUser" value="<?= $userId ?>" class="btn btn-xs btn-outline-success" data-bs-toggle="tooltip" title="Desbloquear usuario">
                                                                                ...
                                                                                <?php else: ?>
                                                                                    <form method="POST" class="d-inline">
                                                                                        <?= csrfField() ?>
                                                                                        <button type="submit" name="blockUser" value="<?= $userId ?>" class="btn btn-xs btn-outline-danger" data-bs-toggle="tooltip" title="Bloquear usuario">
                                                                            ...
                                                                            <?php if (hasPermission('USER_DELETE')): ?>
                                                                                <form method="POST" class="d-inline">
                                                                                    <?= csrfField() ?>
                                                                                    <button type="submit" name="deleteUser" value="<?= $userId ?>" class="btn btn-xs btn-outline-danger" data-bs-toggle="tooltip" title="Eliminar usuario" onclick="return confirm('¿Eliminar este usuario?');">
